...
這個倉庫是 AstrBot 的現代化 fork。此處記錄的命令、部署檔案與相容性邊界均以目前分支為準：Python 3.14+、後端使用 `uv`、前端使用 `corepack pnpm`，且不再為舊相容路徑背書。
...
> AstrBot 需要 Python 3.14 或更高版本。`--python 3.14` 會確保 `uv` 使用 Python 3.14 建立 tool 環境。
...
若希望同時啟動 AstrBot 與 NapCat：

```bash
docker compose -f compose-with-napcat.yml up -d --build
```

更多細節請參考 [使用 Docker 部署 AstrBot](https://docs.astrbot.app/deploy/astrbot/docker.html#%E4%BD%BF%E7%94%A8-docker-%E9%83%A8%E7%BD%B2-astrbot)。
...
前往 [AstrBot Launcher](https://github.com/AstrBotDevs/astrbot-launcher) 下載並安裝。
