...
def _normalize_crontab_day_of_week(day_of_week: str) -> str:
    """Normalize standard crontab weekday numbers for APScheduler.

    Args:
        day_of_week: Day-of-week field from a five-part crontab expression.

    Returns:
        An APScheduler-compatible day-of-week field.

    Raises:
        ValueError: If the weekday values or step are outside the supported
            crontab range.
    """
...
class CronJobManager:
    ...
    async def _woke_main_agent(
        ...
        *,
        ...
        if getattr(runner, "state", AgentState.DONE) == AgentState.ERROR:
            detail = (
