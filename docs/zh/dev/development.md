...
- Python 包要求：3.14 及以上
- 当前开发、Docker 与 CI Python：3.14.7
- Node.js：26.9.0
- Dashboard 与文档 pnpm：12.5.0
- Python 依赖管理：`uv`
...
`make doctor` 检查 Python 3.14.x、`uv`、Node 26.x 和 Dashboard pnpm 12.5.x；在 POSIX 上还检查 `shfmt`、`shellcheck` 和 `hadolint`。`make bootstrap` 使用锁文件同步 Python dev 依赖、根目录 Node 格式化工具和 Dashboard 依赖，但不会安装文档依赖。
