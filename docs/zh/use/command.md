...
监听和发送要求当前身份拥有同一配置下的 `instance_operator` 权限。群管理员、私聊会话所有者身份不能替代它。监听内容对接收会话的所有成员可见；仅转发开始监听之后收到的消息，不读取历史。监听保存在内存中，到期或重启后清空，每人最多 16 项。每次转发都会重新检查权限，撤权后停止。对同一对会话再次 `/session watch` 会重置时长。
