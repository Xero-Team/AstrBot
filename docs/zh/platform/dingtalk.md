...
在 `选择创建方式` 中选择 `扫码一键创建`，使用手机钉钉扫描页面中的二维码，并在钉钉页面中创建或绑定机器人。扫码后若钉钉仍在创建应用（`CREATING`），AstrBot 会继续轮询，直到成功、失败或过期。创建成功后，AstrBot 会自动写入 `ClientID` 和 `ClientSecret`，此时点击 `保存` 即可。
