...
function imageUrlFromContentItem(item: unknown): string | null {
  const record = asRecord(item);
  if (record?.type !== 'image_url') return null;
  const imageUrl = asRecord(record.image_url);
  if (typeof imageUrl?.url !== 'string') return null;
  if (
...
const records = computed(() =>
  props.messages
    .filter((entry) => {
      ...
      return record?.role !== '_checkpoint';
    })
    .map((entry) => {
      const message = asRecord(entry) ?? { content: entry };
        ...
        if (text !== null) {
          if (text.length) {
            parts.push({
              kind: 'text',
              text,
              html: markdownEnabled.value ? renderTextHtml(text) : undefined,
            });
          }
          continue;
        }
        const imageUrl = imageUrlFromContentItem(item);
        if (imageUrl) {
          parts.push({ kind: 'image', text: imageUrl });
          continue;
        }
      ...
      <div class="reading-controls">
        ...
        >
          <button
            ...
            @click="fontSize--"
          >
            <v-icon size="13" aria-hidden="true">mdi-minus</v-icon>
          </button>
          <output
            :aria-label="tm('workspace.preview.fontSize')"
            aria-live="polite"
          ...
          <button
            ...
            @click="fontSize++"
          >
            <v-icon size="13" aria-hidden="true">mdi-plus</v-icon>
          </button>
        </div>
      </div>
    </div>
        ...
        <div class="record-body">
          ...
          <template v-for="(part, partIndex) in record.parts" :key="partIndex">
            ...
            <details v-else class="record-data">
              <summary>
                <v-icon size="13" aria-hidden="true">mdi-chevron-right</v-icon
...
.record-data summary :deep(.v-icon) {
  flex-shrink: 0;
  transition: transform 0.15s;
}
.record-data[open] summary :deep(.v-icon) {
