...
For uploads and URL imports, `chunk_size` is 1-8192 and `chunk_overlap` is 0-8191; the overlap must be smaller than the chunk size. All three import paths use `batch_size` 1-128, `tasks_limit` 1-8, and `max_retries` 0-10. AstrBot accepts at most two knowledge-base ingestion jobs at once, so it can make at most 16 embedding requests concurrently. When capacity is full, the Dashboard returns a retry-later error before it reads the request body, creates an ingestion task, or copies files to the task staging directory. Extracted URL text is also rejected before cleaning or chunking when it exceeds 128 MiB.

When URL content cleaning is enabled, concurrent chunk requests are released at the configured RPM interval instead of waking all waiting requests in one burst. Set `max_rpm=0` to disable this pacing.
