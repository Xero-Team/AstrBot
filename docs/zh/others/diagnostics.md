...
本文用于整理 AstrBot 出现异常时的通用排查方法。遇到问题时，先确定问题发生在哪个阶段，再收集对应日志。
...
看门狗转储会脱敏已识别的 token、密钥字段和 URL，但保留文件路径以便定位卡顿。它不转储局部变量值。源码行里的 Cookie、私聊内容和自定义 secret 不保证被剥离。分享前仍需人工检查。
