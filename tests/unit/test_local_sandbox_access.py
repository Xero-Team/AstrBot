...
def test_runtime_probe_launches_native_sandbox(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(process_sandbox, "get_astrbot_temp_path", lambda: str(tmp_path))

    info = process_sandbox.detect_local_runtime_info()

    assert info["sandbox"]["status"] == "detected", info
...
@pytest.mark.skip(reason="Fork computer tools use ComputerRuntime plus authorization.")
@requires_local_sandbox
