...
`make run` synchronizes the locked runtime environment, builds the Dashboard and documentation, copies them to `data/dist` (docs under `help/`), and then starts both processes. It does not build a Python wheel or sdist. Use `make status` to inspect the processes and `make stop` to stop them. On Linux, `make perf` can sample an already-running backend; see [Linux Development](/en/dev/linux#performance-sampling). `make build-docs` only builds and syncs documentation; `make docs` starts a standalone VitePress preview with base `/`.
`make run` synchronizes the locked runtime environment, builds the Dashboard and documentation, copies them to `data/dist` (docs under `help/`), and then starts both processes. It does not build a Python wheel or sdist. Use `make status` to inspect the processes and `make stop` to stop them. On Linux, `make perf` can sample an already-running backend; see [Linux Development](/en/dev/linux#performance-sampling). `make build-docs` only builds and syncs documentation; `make docs` starts a standalone VitePress preview with base `/`.
