...
- `/session filter <rule_id>`: Show the match/except filter on one directed edge. The filter constrains **forwarded** messages only; it does not constrain `/send`. Permission matches `links` (`session.read` plus the operator extra scope).
...
- `/session health`: Show the session-bridge delivery health snapshot; requires `session.read`. It reports cumulative accepted/partial/failed/unknown/skipped/deauthorized counters, the number of forwards queued right now, and the latest failure per visible rule (one bounded record per rule). Counters are cumulative for the runtime; failing rules are limited to those visible to the actor.
- `/send <UMO> [content]`: Send text and attachments from the same message through the target Bot account; requires `session.send`. An attachment-only body is allowed. This does not register a `reply` command. `match`/`except` filters constrain forwarded messages only and **do not apply to `/send`**.
- `/send [content]`: After `/session connect`, send to the linked target without repeating the UMO. Attachment-only bodies are allowed. `pair` is not a default `/send` target. The target edge's `match`/`except` filter does not constrain this command either.

Watching, connecting, pairing, and sending require the current identity to hold `instance_operator` permission in the configuration shared by both sessions. Group admin or private-session ownership does not grant this access. Forwarded content is visible to everyone in the receiving session. Watches, links, and pairs forward new incoming messages only, without reading history. Empty match and except filters forward every human message. The bridge is a transport plane at ingress: forwarding happens before the pipeline, so `/bot disable`, `/session block`, admission, rate limiting, and content safety neither stop nor retract it; what constrains it is the bridge's own per-delivery re-authorization, shared config, self-message loop prevention, `match`/`except`, and target adapter capabilities. `watch` / `connect` / `pair` do not accept filter flags; set filters afterwards with `/session filter`. Subject filters rebuild `Subject.im` from the source route platform id, the source adapter `self_id`, and `SenderSnapshot.id`. Text filters concatenate `PortablePart` string values and ignore media and `NativeContent`; a media-only body fails `match.text` and does not trip `except.text`. Rules are stored in SQLite, so unexpired watches, all connects, and all pairs survive process restart; expired watches are cleared at startup or when they elapse, and the listener is notified. Limits remain 16 watches, 16 links, and 8 pairs per actor. Authorization is checked again for each forwarded message, so revocation stops delivery. A transient submission failure (platform rejection or unknown status with no accepted fragment) is retried up to two more times after the first attempt; a message counts as forwarded only when at least one fragment is accepted, so a single failure no longer loses it permanently, though an unknown outcome can duplicate at the platform (at-least-once). Running `/session watch` again on the same endpoints keeps the `rule_id` and resets the duration. A watch and a connect on the same direction replace each other. If either target direction is already a pair, `watch` / `connect` is refused and asks for `unpair` first; the pair is not split.

Text and media retain their message-chain order; targets without mixed-content delivery receive separate messages. Cross-platform mentions become text. Quotes use accepted-message ID mappings when available and otherwise become a quote summary. Unavailable attachments and unsupported native content leave text placeholders. Platform cards, private syntax, and mini apps cannot be guaranteed to reproduce on another platform. When the source is a merged forward that carries only a `forward` id and is not yet expanded, the source id is never replayed to the target; it degrades to a localized `[Forwarded messages]` placeholder. When the source is an expanded merged forward and the target declares `forward`, the far side sees a merged-forward card: node authors are the original node `uin` / `name`, and the speaking account is still the destination Bot. When the target does not support `forward` and `t2i.forward_card` is on, the bridge renders the expanded forward as one image card; rendering failure or a disabled switch still yields the labeled transcript.
...
- `/work <task>`: Submit the remaining text to the BTW work loop without a `/chat` prefix or automatic classification. Requires `session.read`, with `btw.enabled` and `btw.work_loop.enabled` enabled on the current profile. The command identity is `builtin_commands:work`. Command quoting rules still apply. The work loop only reads and searches: a step that must write files is handed to a local coding agent named in `btw.work_loop.coding_agents` and runs inside its own task folder. When the task finishes, the conversation loop reports the completion marker and the artifact paths.
