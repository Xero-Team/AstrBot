...
`/session watch` creates an expiring watch from the trusted event identity; command arguments cannot choose an actor. `/session connect` creates an unbounded 1:1 link with the same authorization rules. `/session pair` also requires `session.watch` and writes two directed edges that include the localized source header; `/session unpair` uses the same permission as `disconnect` (`session.read`). Rules persist across restart. On restore, AstrBot rebuilds the subject with `Subject.from_id` and authorizes again; lost permission deletes the edge instead of silently continuing. Authorization is checked again before submission. Stopping a watch, link, or pair, or revoking access, prevents subsequent queued submissions. Forwarded content is visible to everyone in the receiving session. A pair does not forge the source sender's platform identity and does not inject forwards into the target inbound pipeline. Forwarding happens before the event enters the pipeline and is a separate transport plane: `/bot disable`, `/session block`, admission, rate limiting, and content safety do not stop or retract a forward. After a rule exists, `/session filter` can restrict which human messages are forwarded; create commands do not take filter flags. This stage has no Dashboard management surface; see [Built-in commands](./command#cross-session-watches-and-sending).
