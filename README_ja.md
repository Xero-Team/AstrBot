...
<a href="https://github.com/BegoniaHe/AstrBot/issues">Issue</a> ｜
...
このリポジトリは AstrBot のモダン化 fork です。ここで案内するコード、コマンド、デプロイファイル、互換性の前提はこのブランチの実装に合わせています。Python 3.14+、バックエンドは `uv`、ダッシュボードは `corepack pnpm` を前提とし、旧来の互換レイヤーは維持しません。
...
> AstrBot には Python 3.14 以降が必要です。`--python 3.14` を指定すると、`uv` は Python 3.14 で tool 環境を作成します。
...
[AstrBot Launcher](https://github.com/AstrBotDevs/astrbot-launcher) からダウンロードしてインストールしてください。
