...
- `/session connect <UMO>`：把当前会话无期限连接到目标会话，需要 `session.watch`。目标会话的新消息会一直转发，直到 `/session disconnect` 或进程重启。每个主体在每个监听会话只能有一条连接，再次连接会替换目标。省略 UMO 可查看当前连接。
...
监听、连接和发送要求当前身份拥有同一配置下的 `instance_operator` 权限。群管理员、私聊会话所有者身份不能替代它。监听内容对接收会话的所有成员可见；仅转发开始监听之后收到的消息，不读取历史。监听和连接保存在内存中，到期或重启后清空，每人最多 16 条监听和 16 条连接。每次转发都会重新检查权限，撤权后停止。对同一对会话再次 `/session watch` 会重置时长。
