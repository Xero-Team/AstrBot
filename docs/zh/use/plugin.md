...
入口：侧栏 **插件**。配置文件里的可用插件列表在 **配置文件 → 插件配置**。开发请看 [插件开发指南](/dev/star/plugin-new)。
...
配置文件字段 `plugin_set` 默认 `["*"]`，表示使用全部**未被插件页禁用**的插件。空列表表示这个配置文件不使用任何插件。

优先级：

1. 插件页全局禁用：配置文件勾选无效。
2. 配置文件 `plugin_set`：限制该文件能用的插件。
3. [自定义规则](./custom-rules) 的会话插件列表：再按 UMO 收窄。

不同配置文件可以启用不同插件集。改完保存后，该文件的指令 catalog 会重建。
...
2. 插件页已禁用，却在配置文件 `plugin_set` 里反复勾选。
