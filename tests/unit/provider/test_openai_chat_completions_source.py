...
async def test_provider_client_disables_sdk_builtin_retries(overrides, expected_client):
    ...
    expected = (
        AsyncAzureOpenAI if expected_client == "AsyncAzureOpenAI" else AsyncOpenAI
    )
    provider = _make_provider(overrides)
    try:
        assert isinstance(provider.client, expected)
...
async def test_parse_openai_completion_reads_nested_data_choices():
    ...
    try:
        ...
        assert response.id == "nested-completion"
...
@pytest.mark.asyncio
async def test_parse_openai_completion_rejects_invalid_nested_data():
    provider = _make_provider()
    try:
        completion = ChatCompletion.model_construct(
            id="outer-completion",
        ...
        with pytest.raises(EmptyModelOutputError, match="outer-completion"):
            await provider._parse_openai_completion(completion, tools=None)
    finally:
        await provider.terminate()
