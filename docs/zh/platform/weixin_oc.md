...
| 语音     | 是\*         | 是           | \*接收时微信云端会自动转录成文本；发送时转成 Silk |
...
> [!TIP]
> `weixin_oc_token` 和 `weixin_oc_account_id` 会在扫码登录成功后由 AstrBot 自动保存，通常不需要手动填写。
...
部分账号扫码后会要求输入手机微信显示的数字。在创建弹窗中输入该数字并点击继续即可。
...
- 新版微信客户端引用消息可能只携带服务端消息 ID。适配器会用近期收发记录还原引用文本；进程重启后、超出本地近期缓存的历史引用无法还原。
- 出站文本按 4000 字切分发送。开启流式回复后，按约 200 字或 3 秒空闲合并发送。
- 工具调用会发送微信结构化进度消息（开始 / 结果），同时保留输入中状态。
