...
`btw` provides one entry point for the current dual-loop prototype. Every message first enters the conversation loop. With rule-based classification enabled, requests about code, files, commands, search, or research, plus requests beginning with `/work`, are sent to the work loop. The work loop reuses the established Agent and tool execution path; this stage does not introduce Codex, CC, or another specialised executor.
...
- `btw.plugin_routes` lets you choose **Conversation only**, **Work only**, or **Conversation and Work** for every enabled non-system plugin on the **Config** page. No saved entry means **Conversation and Work**, preserving current behaviour.
- `btw.mcp_routes` uses the same choice for every enabled MCP server. Assign MCP servers that use a local process or access sensitive systems to **Work only**.
- `btw.skill_routes` uses the same choice for every enabled Skill. Workspace Skills are only injected into the work loop, even without a saved route.

The conversation loop forcibly disables local computer, sandbox, browser, and filesystem tools, and it also disables file-content extraction. Only the work loop can receive those capabilities. Plugin assignments filter plugin LLM tools, MCP assignments filter all tools provided by each MCP server, and Skill assignments filter which Skill prompts are injected. Existing subagent handoffs receive the same tool routes and cannot regain computer tools from the conversation loop. Plugin Pipeline/Star handlers keep their existing priority. No saved assignment means both loops, so explicitly assign high-privilege MCP servers and Skills to the work loop.
