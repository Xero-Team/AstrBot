...
群聊上下文感知会记录机器人上次回复之后的群聊消息，并在下一次唤醒 LLM 时把这些消息作为额外上下文注入。它只作用于群聊，默认关闭。它不管「这条消息会不会唤醒机器人」，唤醒策略见 [群聊何时会理我](./group-wake)。

该能力位于配置文件的 **配置文件 → 扩展功能 → 群聊上下文感知**。JSON 键仍是历史名称 `provider_ltm_settings`，不要把它当成 Alkaid [长期记忆](./long-term-memory) 的开关。
