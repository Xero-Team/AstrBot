...
Этот репозиторий является модернизированным fork AstrBot. Описанные здесь код, команды, файлы развёртывания и границы совместимости относятся только к текущей ветке: Python 3.14+, `uv` для backend, `corepack pnpm` для dashboard и без сохранения устаревших слоёв совместимости.
...
> Для AstrBot требуется Python 3.14 или новее. Параметр `--python 3.14` гарантирует, что `uv` создаст tool-окружение с Python 3.14.
...
Перейдите в [AstrBot Launcher](https://github.com/AstrBotDevs/astrbot-launcher), чтобы скачать и установить.
