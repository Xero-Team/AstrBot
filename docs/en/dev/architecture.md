...
Inbound routing is a single decision in `WakingCheckStage`: command, LLM, passthrough, or drop. It writes `should_run_command`, `should_run_llm`, `route_kind`, and the explicit `wake_reasons` set onto the event. Command matching runs before LLM access; a matched command normally only runs the command, a bare command group emits help, and an unknown subcommand emits the Orbit diagnostic without falling through to the LLM. Built-in `/work <task>` is the exception: the handler rewrites `message_str` and sets `should_run_llm` plus `btw_force_work` so `ProcessStage` continues into the work loop after the command returns. That path still requires `btw.enabled` and `btw.work_loop.enabled` on the profile, and its `command_id` is `builtin_commands:work`. LLM access is selected from the event's configuration profile through `llm_access`; `command_prefixes` only frames command headers. The derived `is_wake` attribute is not a pipeline gate.
