...
> [!WARNING]
> 需要您的电脑上预先安装好 Python 环境（3.14+），并且将 Python 添加到环境变量中，否则安装器将无法正常工作。

推荐使用上面提到的 AstrBot 一键启动器来部署 AstrBot，因为它更简单、更自动化、更现代化，适合大多数用户。

这个旧安装器属于外部上游工具，不是本 fork 仓库的一部分。本 fork 不保证该脚本内置的下载目标与当前 `BegoniaHe/AstrBot` 分支一致；如果你需要与当前仓库完全对齐的行为，请优先使用本仓库文档中的 `uv`、Docker 或源码部署方式。
...
脚本将自动检测目录下是否有 `AstrBot` 文件夹；如果没有，它会从该外部安装器自身配置的来源下载源码、安装依赖并运行。
...
>
> 如果 WebUI 静态资源缺失或版本不匹配，请重新安装当前启动器版本，或改用本 fork 文档中的仓库部署方式；这里不再维护旧安装器额外下载 `dist.zip` 的流程说明。
