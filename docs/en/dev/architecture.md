...
- `main.py`, `astrbot run`, and the image `CMD` all enter `run_application()`. After `prepare_runtime_environment()` and before Dashboard asset resolution or `create_runtime_services()`, that runner acquires an advisory lock on `data/astrbot.lock`. One process may own a given `data/` directory. Failure exits immediately without opening the database or loading adapters. The operating system releases the lock when the process exits; a leftover `astrbot.lock` file does not hold the lock. `astrbot init` uses the same lock after the install-directory confirmation. If Compose mounts `./data` into a second full instance, that container is expected to fail startup.
