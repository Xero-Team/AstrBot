...
>
> 对于每轮都会变化、内容量中小的提示词，把动态上下文追加到 `req.extra_user_content_parts`，并调用 `mark_as_temp()`。这样不会让 `system_prompt` 每轮变化，也不会把瞬时内容写入会话历史：
>
> ```python
> from astrbot.api.event import filter, AstrMessageEvent
...
> ```
>
> 省略 `mark_as_temp()` 时，该内容块会随用户消息写入会话历史。瞬时状态、敏感数据或较大的长期记忆、知识库和外部查询结果应优先注册为 `llm_tool`，让模型按需读取。
