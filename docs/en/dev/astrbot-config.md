...
Unlisted sessions are controlled by top-level `admission.unlisted_sessions`, default `allow`. `deny` admits groups or DMs that already have an overlay on the canonical session key, or that this profile listed during upgrade. Unlisted senders are controlled by `admission.unlisted_senders`, also default `allow`. `deny` admits only IM subjects that already have a `blocked` or `llm_enabled` overlay. Write those overlays with `/user block`, `/user unblock`, and `/user llm on|off`. WebChat, OneBot `notice` / `request`, and senders with `provider.manage` on the current instance skip this gate. `id_whitelist`, `enable_id_white_list`, and `wl_ignore_admin_*` are gone.
