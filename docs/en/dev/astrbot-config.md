...
After you click **Switch** and confirm, the page saves all pending provider changes on this page to the running profile, then switches the CLI using the selected entry's `id`. A failed save or cancelled verification stops the switch. If saving requires a TOTP code, complete verification and save successfully, then click **Switch** again. Writing the CLI file still requires its own step-up verification.
