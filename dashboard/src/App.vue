<template>
  ...
  <!-- 全局唯一 snackbar -->
  <v-snackbar
v-if="toastStore.current" v-model="snackbarShow" :color="toastStore.current.color"
...
const snackbarShow = computed({
  get: () => Boolean(toastStore.current),
  set: (val) => {
...
onMounted(() => {
  ...
  disposeTrayRestartListener = desktopBridge.onTrayRestartBackend(() => {
    void (async () => {
      try {
