...
export function contextLimit(
  ...
  const context =
    Number(provider?.max_context_tokens) ||
    Number(metadata?.limit?.context) ||
    0;
...
export function providerCapabilityBadges(
  ...
  const modalities = Array.isArray(providerModalities)
    ? providerModalities
    : [];
  const definitions = [
    {
...
function formatCompactNumber(value: number): string {
  const absValue = Math.abs(value);
  const rounded =
    absValue >= 10 ? Math.round(value) : Math.round(value * 10) / 10;
