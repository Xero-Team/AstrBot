...
旧的 `id_whitelist`、`enable_id_white_list` 和 `wl_ignore_admin_*` 已删除，Dashboard 写入这些字段会失败，也不做升级转换：旧配置里残留的这些键会被直接丢弃，准入回退到 `allow`。需要放行的会话请通过 `/bot`、`/llm` 或 Dashboard [自定义规则](./custom-rules) 重新写成覆盖。
