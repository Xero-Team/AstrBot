...
export default {
  ...
  computed: {
    ...
    tableHeaders() {
      return [
        {
          title: this.tm('table.headers.title'),
          key: 'title',
          sortable: true,
          minWidth: '80px',
          width: '200px',
        ...
        {
          title: this.tm('table.headers.createdAt'),
          key: 'created_at',
          sortable: true,
          width: '180px',
        },
        {
          title: this.tm('table.headers.updatedAt'),
          key: 'updated_at',
          sortable: true,
          width: '180px',
        },
      ...
      if (this.isEditingHistory) {
        if (
          await askForConfirmationDialog(
            this.tm('dialogs.view.confirmClose'),
            this.confirmDialog,
          )
        ) {
      ...
      try {
        ...
        if (response.data.status === 'ok') {
          ...
          const index = this.conversations.findIndex(
            (item) =>
              item.user_id === this.editedItem.user_id &&
              item.cid === this.editedItem.cid,
          );
