...
这些方法会再次调用 `authorize()`，要求 `session.watch` 或 `session.send`，且两个会话属于同一配置。监听和连接会持久化，重启后未过期的规则仍在。`remaining_seconds` 按墙钟计算，无期限返回 `0`。不要自己构造 `SessionBridgeManager`。`SessionWatch` 和时长常量可从 `astrbot.api.platform` 导入。`links` / `unlink` 本阶段只做 IM 指令，没有对应的 `SessionBridgeCapability` 方法。当前没有 Dashboard 管理面，插件也不应假设存在对应 HTTP API。
