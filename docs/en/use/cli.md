...
Supported common keys include `timezone`, `log.level`, `dashboard.port`, `dashboard.username`, `dashboard.password`, and `callback_api_base`. Password updates write a PBKDF2 hash to `pbkdf2_password` and clear the legacy `password` field. Do not generate an MD5 value manually.
