...
- `/session connect <UMO>`: Link the current session to the target without expiry; requires `session.watch`. Incoming messages from the target are forwarded until `/session disconnect` or process restart. One link per actor and listener; connecting again replaces the previous target. `/session connect` with no argument shows the current link.
