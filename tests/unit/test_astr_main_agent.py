...
def test_filter_plugin_tools_for_loop_defaults_unassigned_tools_to_work():
    ...
    config = ama.MainAgentBuildConfig(
        btw_enabled=True, tool_call_timeout=60, loop_mode="conversation"
    )
    ...
    work_config = ama.MainAgentBuildConfig(
        btw_enabled=True, tool_call_timeout=60, loop_mode="work"
    )
...
class TestEnsurePersonaAndSkills:
    ...
    def test_conversation_loop_hard_isolation_disabled_with_btw_off(self):
        """With BTW disabled the Agent path matches master: no tool stripping."""
        req = ProviderRequest(
            prompt="hello",
            func_tool=ToolSet([ama.ExecuteShellTool(), ama.FileReadTool()]),
        )
        config = ama.MainAgentBuildConfig(
            tool_call_timeout=60,
            loop_mode="conversation",
