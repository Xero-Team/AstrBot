...
配置文件的 **平台配置** 和 **扩展功能** 里，有一组所有消息平台共用的收发行为。它们在 [唤醒检查](./group-wake) 之后生效：策略已经放行的消息，仍可能被白名单、限流或内容安全丢掉。
...
- **忽略 @ 全体**：`MentionAll` 只是消息链标记，不会单独放行内置 LLM。
- **权限不足时回复**：用户没权限执行指令时是否提示。
- **只打指令前缀是否触发等待**：见 [群聊何时会理我](./group-wake#只打了指令前缀、没有正文)。
