...
`id_whitelist`, `enable_id_white_list`, and `wl_ignore_admin_*` are gone. Dashboard writes that include them fail. They are not converted: an old config that still carries them just drops those keys, and admission falls back to `allow`. Re-add any sessions you want to admit as overlays through `/bot`, `/llm`, or Dashboard [custom rules](./custom-rules).
