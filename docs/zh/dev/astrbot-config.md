...
所选提供商仍须是已配置的对话模型。不存在或类型不适用的循环提供商将回退到事件或会话的模型选择，并输出警告日志，因此单个失效的设置不会让所有请求失败；它不会静默改用另一个循环的模型。已有模型回退和重试设置继续作用于所选主模型。
