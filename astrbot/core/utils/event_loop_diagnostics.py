...
def _redact_watchdog_dump(text: str) -> str:
    """Redact tokens and URLs in a watchdog dump while keeping file paths.

    Args:
        text: Raw watchdog dump text.

    Returns:
        Dump text with recognized secrets and URLs redacted.
    """
    return redact_sensitive_text(text, redact_paths=False)


def _format_watchdog_dump(
    *,
    stalled_for: float,
    event_loop_thread_id: int,
    skip_thread_ids: frozenset[int],
) -> str:
    """Format a redacted dump of Python thread stacks.

    Args:
        stalled_for: Seconds since the last event-loop heartbeat.
        event_loop_thread_id: Thread ident to dump first.
        skip_thread_ids: Thread idents to omit, such as the watchdog worker.

    Returns:
        Redacted watchdog dump text.
    """
    buffer = StringIO()
    buffer.write(f"Event loop stalled for {stalled_for:.3f}s\n")
    frames = sys._current_frames()
    thread_names = {
        thread.ident: thread.name for thread in threading.enumerate() if thread.ident
    }
    thread_ids = [event_loop_thread_id]
    thread_ids.extend(
        thread_id
        for thread_id in frames
        if thread_id != event_loop_thread_id and thread_id not in skip_thread_ids
    )
    ...
    return _redact_watchdog_dump(buffer.getvalue())
...
async def event_loop_watchdog(
    *,
    ...
    Args:
        dump_after: Seconds without a heartbeat before the watchdog dumps stacks.
    ...
    def watch_heartbeat() -> None:
        ...
        while not stop_event.wait(interval):
            ...
            if (
                ...
                or time.monotonic() - observed_heartbeat < dump_after
            ):
                continue
            ...
            try:
                ...
                output.write(
                    _format_watchdog_dump(
                        stalled_for=stalled_for,
                        event_loop_thread_id=event_loop_thread_id,
                        skip_thread_ids=frozenset({threading.get_ident()}),
                    )
                )
                output.flush()
