...
当前可复现开发与 CI 基线为 Python 3.14.7、Node.js 26.9.0 和 pnpm 12.5.0；Python 包元数据允许 3.14 及以上版本。
...
`AdmissionCheckStage` 按 `admission.unlisted_sessions`、`admission.unlisted_senders`、规范会话覆盖和 `Subject.im.id` 上的发送者覆盖决定是否放行。发送者 `blocked` 会丢掉该 IM 主体在本机器人实例上所有会话的事件，只放行 `/user unblock` 和 `/bot status`。WebChat、OneBot `notice` / `request` 以及当前实例上拥有 `provider.manage` 的发送者会跳过。`session_enabled` / `session_blocked` 的指令透传仍由 `SessionStatusCheckStage` 处理。`GroupMessageHistoryStage` 在插件处理前持久化非 WebChat 的入站群消息，供 `GetGroupMessageHistoryTool` 使用；私聊和 WebChat 会跳过。`ProcessStage` 负责插件处理与 Agent 调用；`ResultDecorateStage` 处理前缀、分段、TTS、本地文转图、引用等结果装饰；`RespondStage` 统一调用平台发送接口。流水线同时支持普通异步 stage 和用异步生成器实现的洋葱式前后处理，修改时必须保留停止传播和收尾语义。`SessionStatusCheckStage` 在会话关闭时停止事件，但放行已激活的 `/bot status` 和 `/bot enable`，以便从聊天重新打开会话。没有覆盖时，IM 群聊和私聊的 `session_enabled` / `llm_enabled` / `tts_enabled` 默认关闭；WebChat 默认开启。

会话桥接是独立的传输平面。`Platform.commit_event()` 在事件入队后立即把入口快照交给通过 `add_envelope_observer` 注册的观察者，`AstrBotCoreLifecycle` 在 `platform_manager.initialize()` 之前注册 `SessionBridgeManager.observe`。观察者只跳过 Bot 自身消息并按来源同步入队，能力检查、逐次鉴权、`match`/`except` 与投递都在边 worker 中完成，因此桥接转发不经过 `builtin_stage_classes()` 的 stage 元组，`AdmissionCheckStage`、`SessionStatusCheckStage`、`RateLimitStage`、`ContentSafetyCheckStage` 既不 gate 也不能撤回已经发生的转发。
...
| `session.read_target` / `session.manage_target`   | instance_operator 及以上                                                                         | 否                 |
| `session.block`                                   | instance_operator 及以上                                                                         | 否                 |
