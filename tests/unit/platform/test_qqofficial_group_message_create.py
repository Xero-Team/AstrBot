...
async def test_ws_group_send_by_session_strips_unique_session_suffix():
    ...
    kwargs = adapter.client.api.post_group_message.await_args.kwargs
    assert kwargs["group_openid"] == "group-1"
    assert kwargs["markdown"]["content"] == "unique session hello"
...
@pytest.mark.asyncio
async def test_ws_group_send_by_session_missing_use_markdown_defaults_to_markdown():
    adapter = QQOfficialPlatformAdapter(
        {
            "id": "qq-official-test",
            "appid": "123",
    ...
    await adapter.send_by_session(
        MessageSession("qq_official", MessageType.GROUP_MESSAGE, "group-1"),
        MessageChain(chain=[Plain("legacy archive")]),
    )

    kwargs = adapter.client.api.post_group_message.await_args.kwargs
    assert kwargs["markdown"]["content"] == "legacy archive"
    assert kwargs["msg_type"] == 2
    assert "content" not in kwargs
