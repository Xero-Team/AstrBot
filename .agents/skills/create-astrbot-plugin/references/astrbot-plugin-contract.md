...
- Read `[project].requires-python` from the checkout's `pyproject.toml` before generating a plugin. The current checkout declares `>=3.14`; `.python-version` pins `3.14.7` for development and CI. Generated plugin documentation must repeat the value read from the checkout, not a hand-maintained fallback.
