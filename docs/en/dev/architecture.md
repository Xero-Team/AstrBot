...
- The `astrbot` CLI resolves its CLI runtime root and requires the `.astrbot` marker. Both `astrbot/cli/__main__.py` and `astrbot run` call `runtime_bootstrap.initialize_runtime_bootstrap()` to install the trusted CA. CLI `init` and `run` still do not download or update Dashboard assets, and they do not implement `main.py`'s `--webui-dir`. Changes to startup security, the runtime root, or Dashboard static-asset resolution must still inspect both `main.py` and the CLI.
- `main.py`, `astrbot run`, and the image `CMD` all enter `run_application()`. That runner checks the Python version, then acquires an advisory lock on `data/astrbot.lock`. On POSIX it also `flock`s the `data/` directory itself, so unlinking a held lock file cannot admit a second process. Only after the lock is held does it call `prepare_runtime_environment()` to create data subdirectories, resolve Dashboard assets, and call `create_runtime_services()`. One process may own a given `data/` directory. Failure exits immediately without opening the database or loading adapters. Acquisition failure is distinct from later `Timeout` errors in the held section. The operating system releases the lock when the process exits; a leftover `astrbot.lock` file does not hold the lock. The former CLI-root `<root>/astrbot.lock` is unused. `astrbot init` uses the same lock after the install-directory confirmation. If Compose mounts `./data` into a second full instance, that container is expected to fail startup.
...
- Failed initialization triggers cleanup. Shutdown must tolerate partial initialization and repeated calls. A Dashboard restart only signals shutdown; `InitialLoader` then runs the same `stop()` stack once, and `ProcessRebooter` replaces the process. Importing `astrbot.core` alone must not construct runtime services or access user data.
...
`AdmissionCheckStage` admits events from `admission.unlisted_sessions`, `admission.unlisted_senders`, canonical session overlays, and sender overlays on `Subject.im.id`. Sender `blocked` drops the event in every session of this bot instance except `/user unblock` and `/bot status`. WebChat, OneBot `notice` / `request`, and senders with `provider.manage` on the current instance skip this stage. `session_enabled` / `session_blocked` command passthrough stays in `SessionStatusCheckStage`. `GroupMessageHistoryStage` persists inbound group messages other than WebChat before any plugin handles the event, for `GetGroupMessageHistoryTool`. Direct messages and WebChat skip this stage. `ProcessStage` runs plugin handlers and the Agent. `ResultDecorateStage` applies prefixes, segmentation, TTS, local text-to-image rendering, quoting, and related transformations. `RespondStage` uses the platform's unified send API. The scheduler supports both ordinary async stages and async-generator onion middleware; preserve stop-propagation and finalization semantics. `SessionStatusCheckStage` stops events when the session is disabled, except for activated `/bot status` and `/bot enable` so the session can be turned back on from chat. Unwritten `session_enabled` / `llm_enabled` / `tts_enabled` default off for IM groups and DMs, and on for WebChat.

The session bridge is a separate transport plane. `Platform.commit_event()` hands an ingress snapshot to observers registered with `add_envelope_observer` as soon as the event is enqueued, and `AstrBotCoreLifecycle` registers `SessionBridgeManager.observe` before `platform_manager.initialize()`. The observer only skips the Bot's own messages and enqueues per origin; capability checks, per-delivery authorization, `match`/`except`, and delivery all run in the edge worker. Bridge forwarding therefore does not pass through the `builtin_stage_classes()` tuple: `AdmissionCheckStage`, `SessionStatusCheckStage`, `RateLimitStage`, and `ContentSafetyCheckStage` neither gate it nor retract a forward that already happened.
...
| `session.read_target` / `session.manage_target`   | instance_operator+                                                                                                                  | no                                 |
| `session.block`                                   | instance_operator+                                                                                                                  | no                                 |
| `session.assign`                                  | session_owner+                                                                                                                      | yes across sessions                |
| `session.watch`                                   | instance_operator+; both sessions must share a config                                                                               | no                                 |
| `session.send`                                    | instance_operator+; both sessions must share a config                                                                               | no                                 |
