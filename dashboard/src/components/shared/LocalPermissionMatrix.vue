<template>
  <div
    class="local-permission-matrix"
    :class="{ 'local-permission-matrix--simple': unsupported }"
  >
    <v-progress-linear
    ...
    <dl v-else class="runtime-info">
      <div>
        <dt>
          <v-icon size="14" aria-hidden="true">mdi-monitor</v-icon>
          {{ tm('runtime.os') }}
        </dt>
        <dd>
          {{
      ...
      <div>
        <dt>
          <v-icon size="14" aria-hidden="true">mdi-cpu-64-bit</v-icon>
          {{ tm('runtime.arch') }}
        </dt>
        <dd>{{ runtime.arch || '—' }}</dd>
      </div>
        ...
        <dd>
          {{
            tm(`runtime.status.${runtime.sandbox.status}`, {
              backend:
                runtime.sandbox.backend === 'seatbelt'
                  ? 'Seatbelt'
                  : 'bubblewrap',
              dependency:
                runtime.sandbox.backend === 'seatbelt'
    ...
    <v-alert
      v-if="runtime?.sandbox?.status === 'unavailable'"
      type="warning"
      variant="tonal"
      density="compact"
    >
      <div>{{ tm('runtime.unavailableHint') }}</div>
      <div v-if="runtime.sandbox.error" class="sandbox-error mt-2">
        ...
        <tr v-for="role in roles" :key="role" :aria-label="tm(`roles.${role}`)">
          ...
          <td v-if="unsupported">
            <v-select
              ...
              @update:model-value="
                updatePermission(role, accessPolicies[$event])
              "
            >
              <template #prepend-inner>
                <v-icon
                  v-if="accessModes[role]"
                  :icon="accessIcons[accessModes[role]]"
                  size="18"
                  :class="
                    accessModes[role] === 'full'
                      ? 'text-warning'
              ...
              <template #item="{ props: itemProps, item }">
                <v-list-item
                  ...
                  :lines="false"
                  role="option"
                  :aria-selected="accessModes[role] === item.value"
                >
                  <template #prepend>
                    <v-icon
                      :icon="accessIcons[item.value]"
                      size="18"
                      class="mr-3"
                      aria-hidden="true"
                    />
          ...
          <template v-else>
            <td class="permission-toggle">
              <v-checkbox-btn
                ...
                density="compact"
                @update:model-value="
                  updatePermission(role, { allow_execution: Boolean($event) })
                "
              />
              <v-icon
                v-if="permissionLocks[role].execution"
            ...
            <td class="permission-toggle">
              <v-checkbox-btn
                ...
                density="compact"
                @update:model-value="
                  updatePermission(role, { allow_network: Boolean($event) })
                "
              />
              <v-icon
                v-if="permissionLocks[role].network"
                class="permission-lock"
                size="12"
                aria-hidden="true"
              >
            ...
            <td>
              <v-select
                ...
                :menu-props="{ rounded: 'lg', maxWidth: 420 }"
                @update:model-value="
                  updatePermission(role, { filesystem_scope: $event })
                "
              >
                <template #item="{ props: itemProps, item }">
                  <v-list-item
                    ...
                    role="option"
                    :aria-selected="
                      policy(role).filesystem_scope === item.value
                    "
                  >
                    <template #subtitle>
                      <div class="scope-hint">
...
const { tm } = useModuleI18n(
  'features/config-metadata.ai_group.agent_computer_use.local_permissions',
);
const runtime = ref(null);
const runtimeLoading = ref(true);
const unsupported = computed(
  () => runtime.value?.sandbox?.status === 'unsupported',
);
...
function policy(role) {
  ...
  resolved.allow_execution =
    resolved.filesystem_scope !== 'none' && resolved.allow_execution === true;
  resolved.allow_network =
    resolved.allow_execution && resolved.allow_network === true;
  return resolved;
}
...
function updatePermission(role, changes) {
  ...
  if (
    updatedRole.filesystem_scope === 'none' ||
    (sandboxUnavailable.value && changes.filesystem_scope === 'workspace')
  ) {
...
const accessIcons = {
  none: 'mdi-lock-outline',
  files: 'mdi-folder-open',
  full: 'mdi-shield-alert-outline',
};
const permissionLocks = computed(() =>
  Object.fromEntries(
    roles.map((role) => {
      ...
      return [
        role,
        {
          execution:
            !runtime.value ||
            current.filesystem_scope === 'none' ||
            (sandboxUnavailable.value &&
              current.filesystem_scope === 'workspace' &&
              !current.allow_execution),
          network:
            !runtime.value ||
            !current.allow_execution ||
